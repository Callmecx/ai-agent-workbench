...
export function refreshBase(id: string) {
  const base = db.bases.find((b) => b.id === id);
  if (!base) return;
  const docs = db.documents.filter((d) => d.knowledgeBaseId === id);
  base.documentCount = docs.length;
  base.chunkCount = docs.reduce((sum, d) => sum + d.chunkCount, 0);
  base.embeddingStatus = docs.some((d) => d.status === 'FAILED')
    ? 'FAILED'
    : docs.length && docs.every((d) => d.status === 'READY')
      ? 'READY'
      : 'PENDING';
}
export async function processDocument(id: string) {
  ...
  try {
    for (const status of ['PARSING', 'EMBEDDING', 'READY'] as const) {
      ...
      if (!doc) return;
      if (status === 'EMBEDDING' && !doc.text.trim()) {
        doc.status = 'FAILED';
        doc.embeddingStatus = 'FAILED';
        doc.error =
          'Document has no readable text. Upload a non-empty TXT/MD file or retry the mock PDF simulation.';
        refreshBase(doc.knowledgeBaseId);
        save();
        return;
      }
      doc.status = status;
      doc.embeddingStatus = status === 'PARSING' ? 'PENDING' : status;
      if (status === 'READY') {
        ...
        for (let offset = 0, index = 0; offset < doc.text.length; offset += 420, index++) {
          ...
          db.chunks.push({
            ...
            text,
            metadata: { source: doc.name, index, characters: text.length, mock: true },
          });
        }
...
export function searchKnowledge(
  ...
  return db.chunks
    .filter((c) => c.knowledgeBaseId === baseId)
    .map((chunk) => {
      const matches = terms.filter((term) => chunk.text.toLowerCase().includes(term)).length;
      return {
        rank: 0,
        score: terms.length ? Math.round((matches / terms.length) * 100) / 100 : 0,
        chunk,
