...
export const config = {
  ...
  apiKey: process.env.AI_API_KEY || '', realEnabled: process.env.ALLOW_REAL_API === 'true',
  models: (process.env.AI_MODELS || 'gpt-4.1-mini').split(',').map((id) => id.trim()).filter(Boolean),
