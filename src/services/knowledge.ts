...
export const knowledgeService = {
  list: (signal?: AbortSignal) =>
    unwrap<{ bases: KnowledgeBase[]; documents: KnowledgeDocument[] }>(
      api.get('/knowledge', { signal }),
    ),
  create: (name: string) => unwrap<KnowledgeBase>(api.post('/knowledge', { name })),
  rename: (id: string, name: string) =>
    unwrap<KnowledgeBase>(api.patch(`/knowledge/${id}`, { name })),
  ...
  upload: (id: string, file: File, signal?: AbortSignal) => {
    ...
    return unwrap<KnowledgeDocument>(api.post('/knowledge/upload', form, { signal }));
  },
  ...
  search: (
    data: { query: string; knowledgeBaseId: string; topK: number; threshold: number },
    signal: AbortSignal,
  ) =>
    unwrap<{ results: RetrievalResult[]; latencyMs: number; method: string }>(
      api.post('/knowledge/search', data, { signal }),
    ),
};
