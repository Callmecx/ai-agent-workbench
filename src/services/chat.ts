...
export const chatService = {
  ...
  save: (conversation: Conversation) =>
    unwrap<Conversation>(api.put(`/conversations/${conversation.id}`, conversation)),
...
export async function streamChat(
  ...
  if (!response.ok) {
    ...
    throw {
      ...(body.error || { code: 'HTTP_ERROR', message: `Request failed (${response.status}).` }),
  ...
  for await (const frame of readSSE(response.body)) {
    const packet = JSON.parse(frame) as APIResponse<StreamEvent>;
    if (!packet.success) throw { ...packet.error, requestId: packet.requestId };
    if (packet.data) {
      onEvent(packet.data);
      if (packet.data.type === 'done') done = true;
    }
  }
