...
export function selectContext(
  ...
  const history = messages.filter(
    (m) => m.role !== 'system' && m.content && m.status !== 'ERROR' && m.status !== 'ABORTED',
  );
  ...
  for (let i = groups.length - 1; i >= 0; i--) {
    ...
    const size = group.reduce((sum, m) => sum + estimateTokens(m.content) + 4, 0);
    if (used + size > budget) {
      if (!selected.length)
        throw new Error(
          'Your latest message and system prompt exceed the context budget. Increase Context Length or lower Max Tokens.',
        );
      break;
    }
